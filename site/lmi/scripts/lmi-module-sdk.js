(function(){
  ...
  function broadcastWallpaper(wallpaper){ wallpaper=String(wallpaper||'').trim(); if(wallpaper)localStorage.setItem('LMI_WALLPAPER',wallpaper); else localStorage.removeItem('LMI_WALLPAPER'); if(window.parent&&window.parent!==window) window.parent.postMessage({type:'LMI_SET_WALLPAPER',wallpaper},location.origin); }
