(function(){
  ...
  function normalizeAssetUrl(src){
    try{
      return window.parent?.LMI_NORMALIZE_ASSET_URL
        ? window.parent.LMI_NORMALIZE_ASSET_URL(src)
        : String(src || '').trim();
    }catch{
      return String(src || '').trim();
    }
  }
  function broadcastWallpaper(wallpaper){ wallpaper=normalizeAssetUrl(wallpaper); if(wallpaper)localStorage.setItem('LMI_WALLPAPER',wallpaper); else localStorage.removeItem('LMI_WALLPAPER'); if(window.parent&&window.parent!==window) window.parent.postMessage({type:'LMI_SET_WALLPAPER',wallpaper},location.origin); }
  ...
  window.LMI={ready,getContext,api,setRelayUrl,refreshApps,broadcastTheme,broadcastWallpaper,applyTheme,setBusy,broadcastCurrency,money:{format:formatMoney,toDisplay,current:currentCurrency,init:initCurrency,save:saveCurrency,key:CURRENCY_KEY}};
})();
