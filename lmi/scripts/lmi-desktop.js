(async function(){
  ...
  function desktopBounds(){ const desk=$('desktopIcons'); const r=desk?.getBoundingClientRect(); return r||{left:0,top:0,width:window.innerWidth,height:window.innerHeight}; }
  function renderDesktop(){
    ...
    visibleApps().forEach((app,i)=>{
      ...
      b.addEventListener('pointerdown',ev=>{
        if(ev.button!==0)return; ev.preventDefault(); moved=false; suppressClick=false; downAt=Date.now();
        document.querySelectorAll('.desktop-icon').forEach(x=>x.classList.remove('selected')); b.classList.add('selected');
        const br=b.getBoundingClientRect(); const ox=ev.clientX-br.left, oy=ev.clientY-br.top; const sx=ev.clientX, sy=ev.clientY;
        function mv(e){
          const d=desktopBounds();
          const x=clamp(e.clientX-d.left-ox,0,Math.max(0,d.width-b.offsetWidth));
          const y=clamp(e.clientY-d.top-oy,0,Math.max(0,d.height-b.offsetHeight));
