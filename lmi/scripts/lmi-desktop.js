(async function(){
  ...
  function desktopBounds(){ const host=$('desktopIcons')||document.querySelector('.workspace'); const r=host?.getBoundingClientRect(); if(r && r.width>40 && r.height>40) return r; const w=document.querySelector('.workspace')?.getBoundingClientRect(); return w||{left:0,top:0,width:window.innerWidth,height:Math.max(0,window.innerHeight-82)}; }
