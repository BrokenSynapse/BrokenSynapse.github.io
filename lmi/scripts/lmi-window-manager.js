(function(){
  ...
  function makeInteractive(win,app){
    ...
    bar.addEventListener('pointerdown',e=>{
      ...
      function up(ev){ try{bar.releasePointerCapture?.(e.pointerId)}catch{} document.removeEventListener('pointermove',move); document.removeEventListener('pointerup',up); save(app,win); }
      document.addEventListener('pointermove',move); document.addEventListener('pointerup',up);
    });
    win.querySelectorAll('.resize-handle').forEach(handle=>handle.addEventListener('pointerdown',e=>{
      e.preventDefault(); e.stopPropagation(); focusWindow(win); if(win.classList.contains('maxed')) return;
      const dir=handle.dataset.dir||'se'; const start=rectFromWin(win), sx=e.clientX, sy=e.clientY, b=bounds();
      function move(ev){
        ...
        w=Math.min(w,b.w-x); h=Math.min(h,b.h-y); x=clamp(x,0,Math.max(0,b.w-MIN_W)); y=clamp(y,0,Math.max(0,b.h-MIN_H));
        applyRect(win,{x,y,w,h});
      }
      function up(){ document.removeEventListener('pointermove',move); document.removeEventListener('pointerup',up); save(app,win); }
      document.addEventListener('pointermove',move); document.addEventListener('pointerup',up);
    }));
