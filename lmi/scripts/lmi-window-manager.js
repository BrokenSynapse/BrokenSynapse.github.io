(function(){
  ...
  function makeInteractive(win,app){
    ...
    win.querySelectorAll('.resize-handle').forEach(handle=>handle.addEventListener('pointerdown',e=>{
      ...
      const dir=handle.dataset.dir||'se'; const start=rectFromWin(win), sx=e.clientX, sy=e.clientY, b=bounds(); frameShield(true); handle.setPointerCapture?.(e.pointerId);
      function move(ev){
      ...
      function up(){ try{handle.releasePointerCapture?.(e.pointerId)}catch{} frameShield(false); document.removeEventListener('pointermove',move); document.removeEventListener('pointerup',up); save(app,win); }
      document.addEventListener('pointermove',move); document.addEventListener('pointerup',up);
    }));
